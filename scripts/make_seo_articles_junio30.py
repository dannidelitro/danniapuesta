...
articles = [
    {
        "slug": "modelamiento-friccion-competitiva-regresion-tarjetas-apuestas",
        "title": "Índice de Fricción Competitiva: Proyectando el Mercado de Tarjetas",
        "desc": "Aprende cómo utilizar el índice de fricción táctica y modelos de regresión lineal para dominar los mercados disciplinarios (Over/Under de Tarjetas).",
        "h1": "Tarjetas y Fricción: El Mercado Disciplinario",
        "body": """
        <p>Apostar a las tarjetas o amonestaciones no debe basarse en el carácter de los jugadores, sino en el contexto matemático y situacional del encuentro. Aquí es donde el <strong>Índice de Fricción Táctica ($I_f$)</strong> marca la diferencia.</p>
        <h2>¿Qué es el Índice de Fricción?</h2>
        <p>Este índice cuantifica la tensión acumulada de un partido. Se dispara cuando dos equipos igualan en puntaje muy cerca de zonas críticas, como el descenso o el campeonato. A menor diferencia de puntos y mayor urgencia, más altas son las probabilidades empíricas de faltas tácticas para interrumpir transiciones.</p>
        <h2>Regresión Lineal Generalizada (GLM)</h2>
        <p>Al contrario de los goles (eventos raros modelados con Poisson), las tarjetas son eventos más frecuentes pero que sufren de <em>sobredispersión</em> (una falta grave puede generar múltiples tarjetas en un altercado). La Regresión Lineal Generalizada con distribución binomial negativa es el estándar de oro cuantitativo para aislar el promedio real de tarjetas esperadas en ligas con árbitros permisivos o estrictos.</p>
        <h2>Penalización de Anomalías Históricas</h2>
        <p>Un error grave de los principiantes es observar únicamente las últimas 5 jornadas. Si un equipo acaba de asegurar su permanencia, su índice de fricción decaerá dramáticamente en la última jornada (relajación competitiva). Un inversor astuto sabe cuándo penalizar el historial reciente y abstenerse de apostar.</p>
        """
    },
    {
        "slug": "poisson-bivariado-prediccion-goles-apuestas",
        "title": "Distribución Bivariada de Poisson: El Núcleo de la Predicción de Goles",
        "desc": "Aprende cómo el modelo bivariado de Poisson ajusta la correlación entre goles locales y visitantes para predecir marcadores exactos en apuestas deportivas.",
        "h1": "La Precisión Matemática: Distribución Bivariada de Poisson",
        "body": """
        <p>La modelación de eventos de alta precisión en el fútbol profesional exige metodologías estadísticas robustas. Asumir que los goles de ambos equipos son eventos independientes es un error de novato. Aquí es donde entra en juego la <strong>distribución bivariada de Poisson modificada</strong>.</p>
        <h2>Correlación y Dispersión</h2>
        <p>Este modelo permite calcular las probabilidades de un marcador exacto ajustando la correlación entre los goles anotados por el equipo local y el visitante mediante un parámetro de dispersión. Si el equipo local marca temprano, la táctica del visitante cambia drásticamente; la distribución bivariada es la única que captura matemáticamente esta interdependencia.</p>
        <h2>Goles Esperados (xG) y Decaimiento Temporal</h2>
        <p>Las fórmulas no utilizan simples promedios históricos. Se basan en <em>Goles Esperados (xG)</em> ajustados por índices de fuerza ofensiva y defensiva ponderados por un <strong>factor de decaimiento temporal</strong>. Este factor otorga mucha más relevancia a los partidos más recientes, neutralizando el ruido estadístico de la obsolescencia.</p>
        <h2>Simulaciones de Montecarlo</h2>
        <p>Una vez calibrados los parámetros de Poisson y la Regresión Lineal Generalizada (GLM) para eventos continuos como córners, se ejecutan más de <strong>10,000 iteraciones de Montecarlo</strong> por partido. El resultado es el porcentaje probabilístico exacto para mercados como 1X o BTTS, detectando instantáneamente el codiciado Valor Esperado (EV+).</p>
        """
    },
    {
