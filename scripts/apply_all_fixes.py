...
for f in files:
    ...
    if "window.goNovibet =" not in content:
        ...
        # replace existing
        content = re.sub(r'<script>.*?window\.goNovibet.*?</script>', script_tag, content, flags=re.DOTALL)
    ...
    # 3. Add to Header
    header_btn = f'\\n        <a class="btn-novibet-pro" style="margin-left:8px;" href="javascript:void(0)" onclick="goNovibet()">🎁 Bono Novibet</a>'
    if "Bono Novibet" not in content and '<div class="header-actions">' in content:
        content = content.replace('      </div>\n    </div>\n  </header>', header_btn + '\n      </div>\n    </div>\n  </header>')
        
    # 4. Add to CTA Box
    cta_btn = f'<br><br><a class="btn-novibet-pro" style="margin-top:10px;" href="javascript:void(0)" onclick="goNovibet()">💸 Reclamar Bono Novibet</a>'
