...
articles = [
    ...
    },
    {
        ...
        <p>Poisson es ideal para predecir el número de eventos independientes que ocurren en un intervalo de tiempo fijo (como los 90 minutos de un partido). El input clave es <strong>Lambda ($\lambda$)</strong>, que representa la expectativa media de goles calculada a partir del historial del local atacando y el visitante defendiendo.</p>
        <h2>Calculando Resultados Exactos</h2>
        <p>Supongamos que un duelo de alta intensidad en el Brasileirão tiene un $\lambda$ proyectado de 3.4 goles en total. Utilizando la fórmula <code>P(X = k) = (e^(-λ) * λ^k) / k!</code>, podemos calcular que la probabilidad de que haya <em>exactamente</em> 3 goles es cercana al 21.8%. Sumando las probabilidades de 3, 4, 5 y más goles, obtenemos la probabilidad acumulada para el mercado de "Más de 2.5 goles".</p>
...
# Generate or update all 15 articles
for art in articles:
...
# Update blog/index.html to include all 15 links cleanly (newest first)
...
with open(blog_index_path, "w", encoding="utf-8") as f:
    f.write(idx_content)
print("Blog index updated with all 15 Evergreen articles.")
